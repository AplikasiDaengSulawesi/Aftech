...
<style>
    ...
    .pin-code-text { font-family: 'Courier New', Courier, monospace; font-weight: 900; color: #1A237E; letter-spacing: 2px; font-size: 16px; }
...
<body>
    ...
    <div id="main-wrapper">
                        ...
                        <div class="card shadow-sm border-0" style="border-radius: 15px;">
                            <div class="card-body p-3">
                                ...
                                <div class="nav flex-column nav-pills settings-nav" id="v-pills-tab" role="tablist">
                                    ...
                                    <button class="nav-link text-start" data-bs-toggle="pill" data-bs-target="#tab-machine-group"><i class="la la-industry"></i> Mesin & Kapasitas</button>
                                    ...
                                    <button class="nav-link text-start" data-bs-toggle="pill" data-bs-target="#tab-pin"><i class="la la-key"></i> Master PIN Mobile</button>
                    ...
                    <div class="col-xl-9 col-lg-8">
                        <div class="tab-content">
                            ...
                            <div class="tab-pane fade" id="tab-machine-group">
                                <div class="card content-card shadow-sm">
                                    <div class="card-header border-0 d-flex justify-content-between align-items-center">
                                        <h4 class="card-title text-black">Mesin & Kapasitas Produksi</h4>
                                        <div class="btn-group">
                                            <button onclick="openMachineModal()" class="btn btn-primary btn-sm me-2">+ Mesin</button>
                                            <button onclick="openQtyModal()" class="btn btn-outline-primary btn-sm bg-white">+ Kapasitas</button>
                                        </div>
                                    </div>
                                    <div class="card-body p-0">
                                        <div class="table-responsive"><table class="table shadow-hover">
                                            <thead><tr><th class="ps-4">NAMA MESIN</th><th>STATUS</th><th>DAFTAR KAPASITAS (QTY)</th><th class="text-center">AKSI</th></tr></thead>
                            ...
                            <!-- TAB: PIN -->
                            <div class="tab-pane fade" id="tab-pin">
                                <div class="card content-card shadow-sm">
                                    <div class="card-header border-0 d-flex justify-content-between align-items-center">
                                        <div><h4 class="card-title text-black mb-0">Master PIN Mobile</h4><p class="small text-muted mb-0">PIN Otoritas Reset Aplikasi</p></div>
                                        <button onclick="openPinModal()" class="btn btn-primary btn-sm">+ PIN Baru</button>
                                    </div>
                                    <div class="card-body p-0">
                                        <div class="table-responsive"><table class="table shadow-hover">
                                            <thead><tr><th class="ps-4">KODE PIN</th><th>KETERANGAN</th><th class="text-center">AKSI</th></tr></thead>
                                            <tbody id="tbody-pin"></tbody>
                                        </table></div>
                                    </div>
                                </div>
                            </div>
        ...
        <!-- MODALS -->
        <div class="modal fade" id="modalMachine" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content border-0 shadow-lg"><form id="formMachine">
            <div class="modal-header bg-primary text-white border-0"><h5>Data Mesin Produksi</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
            <div class="modal-body p-4">
                <input type="hidden" name="id" id="machine_id">
                <div class="mb-3"><label class="font-w600 small">Nama Mesin</label><input type="text" name="name" id="machine_name" class="form-control" required></div>
                <div class="mb-2"><label class="font-w600 small">Status Aktivitas</label><select name="status" id="machine_status" class="form-control default-select"><option value="active">ACTIVE (Siap Produksi)</option><option value="maintenance">MAINTENANCE (Dalam Perbaikan)</option></select></div>
            </div>
            <div class="modal-footer border-0"><button type="submit" class="btn btn-primary btn-sm shadow w-100">Simpan Mesin</button></div>
        </form></div></div></div>

        <div class="modal fade" id="modalPin" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content border-0 shadow-lg"><form id="formPin"><div class="modal-header bg-primary text-white border-0"><h5>Data PIN Mobile</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><div class="modal-body p-4"><input type="hidden" name="id" id="pin_id"><div class="mb-3"><label class="font-w600 small">PIN Code</label><input type="text" name="pin_code" id="pin_code" class="form-control text-center font-w900" style="font-size:32px; letter-spacing:8px; color:#1A237E;" required></div><div class="mb-2"><label class="font-w600 small">Keterangan Otoritas</label><input type="text" name="note" id="pin_note" class="form-control" required></div></div><div class="modal-footer border-0"><button type="submit" class="btn btn-primary btn-sm shadow w-100">Simpan PIN</button></div></form></div></div></div>
        ...
        <div class="modal fade" id="modalTemplate" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content border-0 shadow-lg"><form id="formTemplate">
            <div class="modal-header bg-primary text-white border-0"><h5 class="text-white" style="color: #ffffff !important;">Data Template Produksi</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
            <div class="modal-body p-4">
                <input type="hidden" name="id" id="template_id">
                <div class="mb-3"><label class="font-w600 small">Nama Template</label><input type="text" name="template_name" id="template_name" class="form-control" placeholder="Contoh: Produksi Cup 120ml" required></div>
                <div class="row g-2 mb-3">
                    <div class="col-6"><label class="font-w600 small">Item</label>
                        <select name="item" id="template_item" class="form-control default-select" onchange="updateTemplateSizes(this.value)">
                            <option value="">Pilih Item</option>
                            <?php foreach($items as $i) echo "<option value='{$i['name']}'>{$i['name']}</option>"; ?>
                        </select>
                    </div>
                    <div class="col-3"><label class="font-w600 small">Size</label>
                        <input type="text" name="size" id="template_size" list="list_sizes" class="form-control" placeholder="0" required>
                        <datalist id="list_sizes"></datalist>
                    </div>
                    <div class="col-3"><label class="font-w600 small">Unit</label>
                        <select name="unit" id="template_unit" class="form-control default-select">
                            <?php foreach($units as $u) echo "<option value='{$u['name']}'>{$u['name']}</option>"; ?>
                        </select>
                    </div>
                </div>
                <div class="row g-2 mb-2">
                    <div class="col-6"><label class="font-w600 small">Mesin</label>
                        <select name="machine" id="template_machine" class="form-control default-select" onchange="updateTemplateQtys(this.value)">
                            <option value="">Pilih Mesin</option>
                            <?php foreach($machines as $m) echo "<option value='{$m['name']}'>{$m['name']}</option>"; ?>
                        </select>
                    </div>
                    <div class="col-6"><label class="font-w600 small">Shift</label>
                        <select name="shift" id="template_shift" class="form-control default-select">
                            <?php foreach($shifts as $s) echo "<option value='{$s['name']}'>{$s['name']}</option>"; ?>
                        </select>
                    </div>
                    <div class="col-12 mt-2"><label class="font-w600 small">Isi/Kapasitas (Qty)</label>
                        <input type="text" name="quantity" id="template_quantity" list="list_qtys" class="form-control" placeholder="0" required>
                        <datalist id="list_qtys"></datalist>
                    </div>
                </div>
            </div>
            <div class="modal-footer border-0"><button type="submit" class="btn btn-primary btn-sm shadow w-100">Simpan Template</button></div>
        </form></div></div></div>
    </div>
    ...
    <script>
        ...
        window.loadMasterData = async function() {
            const types = ['item', 'machine', 'user', 'customer', 'unit', 'pin', 'size', 'quantity', 'shift', 'template'];
            let allData = {};
            ...
            window.allMasterData = allData; // Simpan global untuk modal template

            // Render Grouped Produk
            ...
            if(tbodyItem) {
                ...
                allData.item.forEach(item => {
                    ...
                    tbodyItem.insertAdjacentHTML('beforeend', `<tr><td class="ps-4 font-w600 text-black">${item.name}</td><td><span class="badge badge-light">${item.unit_name || '-'}</span></td><td><div class="badge-group">${sizeHtml || '<small class="text-muted">No Size</small>'}</div></td><td class="text-center"><div class="d-flex justify-content-center"><button onclick='openItemModal(${JSON.stringify(item)})' class="btn btn-primary btn-xs sharp me-1 shadow-sm"><i class="fa fa-pencil"></i></button><button onclick="deleteMaster('item', ${item.id})" class="btn btn-danger btn-xs sharp shadow-sm"><i class="fa fa-trash"></i></button></div></td></tr>`);
                });
            }

            // Render Grouped Mesin
            ...
            if(tbodyMachine) {
                ...
                allData.machine.forEach(m => {
                    ...
                    tbodyMachine.insertAdjacentHTML('beforeend', `<tr><td class="ps-4 font-w600 text-black">${m.name}</td><td><span class="badge badge-status ${st} text-white">${m.status.toUpperCase()}</span></td><td><div class="badge-group">${qtyHtml || '<small class="text-muted">No Qty</small>'}</div></td><td class="text-center"><div class="d-flex justify-content-center"><button onclick='openMachineModal(${JSON.stringify(m)})' class="btn btn-primary btn-xs sharp me-1 shadow-sm"><i class="fa fa-pencil"></i></button><button onclick="deleteMaster('machine', ${m.id})" class="btn btn-danger btn-xs sharp shadow-sm"><i class="fa fa-trash"></i></button></div></td></tr>`);
                });
            }

            renderTable('user', allData.user);
            renderTableWithPagination('customer', allData.customer);
            renderTable('pin', allData.pin);
            renderTable('unit', allData.unit);
            renderTable('shift', allData.shift);
            renderTableWithPagination('template', allData.template);
        }

        // Auto-Polling Master Data
        setInterval(() => {
            if (!document.hidden) loadMasterData();
        }, 5000);
        ...
        function renderTable(type, data) {
            ...
            data.forEach(row => {
                let html = '';
                if(type === 'user') html = `<tr><td class="ps-4 font-w600">${row.username}</td><td>${row.full_name}</td><td><span class="badge badge-role bg-light text-primary border border-primary font-w700" style="font-size:10px;">${row.role.toUpperCase()}</span></td><td class="text-center"><button onclick='openUserModal(${JSON.stringify(row)})' class="btn btn-primary btn-xs sharp me-1 shadow-sm"><i class="fa fa-pencil"></i></button><button onclick="deleteMaster('user', ${row.id})" class="btn btn-danger btn-xs sharp shadow-sm"><i class="fa fa-trash"></i></button></td></tr>`;
                else if(type === 'pin') html = `<tr><td class="ps-4 pin-code-text">${row.pin_code}</td><td>${row.note}</td><td class="text-center"><button onclick='openPinModal(${JSON.stringify(row)})' class="btn btn-primary btn-xs sharp me-1 shadow-sm"><i class="fa fa-pencil"></i></button><button onclick="deleteMaster('pin', ${row.id})" class="btn btn-danger btn-xs sharp shadow-sm"><i class="fa fa-trash"></i></button></td></tr>`;
                else if(type === 'unit') html = `<tr><td class="ps-4 font-w600 text-black">${row.name}</td><td class="text-center"><button onclick="openModal('unit', ${JSON.stringify(row).replace(/"/g, '&quot;')})" class="btn btn-primary btn-xs sharp me-1 shadow-sm"><i class="fa fa-pencil"></i></button><button onclick="deleteMaster('unit', ${row.id})" class="btn btn-danger btn-xs sharp shadow-sm"><i class="fa fa-trash"></i></button></td></tr>`;
                else if(type === 'shift') html = `<tr><td class="ps-4 font-w600 text-black">${row.name}</td><td class="text-center"><button onclick="openModal('shift', ${JSON.stringify(row).replace(/"/g, '&quot;')})" class="btn btn-primary btn-xs sharp me-1 shadow-sm"><i class="fa fa-pencil"></i></button><button onclick="deleteMaster('shift', ${row.id})" class="btn btn-danger btn-xs sharp shadow-sm"><i class="fa fa-trash"></i></button></td></tr>`;
        ...
        function renderTableWithPagination(type, data) {
            ...
            paginatedData.forEach(row => {
                let html = '';
                if(type === 'customer') html = `<tr><td class="ps-4 font-w700 text-black">${row.name}</td><td><div class="text-primary font-w600" style="font-size:12px;"><i class="fa fa-phone-alt me-1"></i> ${row.contact || '-'}</div><small class="text-muted d-block" style="line-height:1.2;">${row.address || '-'}</small></td><td class="text-center"><span class="badge badge-light border font-w800 text-dark" style="font-size:12px;">${row.total_orders || 0}</span></td><td class="text-center"><button onclick='openCustomerModal(${JSON.stringify(row)})' class="btn btn-primary btn-xs sharp me-1 shadow-sm"><i class="fa fa-pencil"></i></button><button onclick="deleteMaster('customer', ${row.id})" class="btn btn-danger btn-xs sharp shadow-sm"><i class="fa fa-trash"></i></button></td></tr>`;
                else if(type === 'template') html = `<tr><td class="ps-4 font-w700 text-primary">${row.template_name}</td><td><div class="text-black font-w600">${row.item}</div><small class="text-muted">${row.size} ${row.unit}</small></td><td><div class="text-black font-w600">${row.machine}</div><small class="text-muted">${row.shift} &bull; Qty: ${row.quantity}</small></td><td class="text-center"><button onclick='openTemplateModal(${JSON.stringify(row).replace(/"/g, '&quot;')})' class="btn btn-primary btn-xs sharp me-1 shadow-sm"><i class="fa fa-pencil"></i></button><button onclick="deleteMaster('template', ${row.id})" class="btn btn-danger btn-xs sharp shadow-sm"><i class="fa fa-trash"></i></button></td></tr>`;
                tbody.insertAdjacentHTML('beforeend', html);
            });
            ...
            if(info) info.innerText = `Showing ${Math.min(startIndex + 1, totalItems)} to ${Math.min(startIndex + itemsPerPage, totalItems)} of ${totalItems}`;
            ...
            if(controls) {
                ...
                for (let i = 1; i <= totalPages; i++) {
                    controls.insertAdjacentHTML('beforeend', `<li class="page-item ${currentPage === i ? 'active' : ''}"><a class="page-link" onclick="changePage('${type}', ${i})">${i}</a></li>`);
                }
        ...
        async function handleAJAXSave(e, type, modalId) {
            e.preventDefault();
            const res = await fetch(`../api/admin_manage.php?action=save&type=${type}`, { method: 'POST', body: new FormData(e.target) });
            const data = await res.json();
            if(data.status === 'success') { 
                bootstrap.Modal.getInstance(document.getElementById(modalId)).hide(); 
                toastr.success(`Data ${type.charAt(0).toUpperCase() + type.slice(1)} berhasil disimpan ke sistem.`); 
                loadMasterData(); 
            } else {
        ...
        window.deleteMaster = function(type, id) {
            Swal.fire({ 
                ...
                icon: 'warning', showCancelButton: true, confirmButtonColor: '#D50000', confirmButtonText: 'Ya, Hapus' 
            }).then(async (res) => {
                if(res.isConfirmed) {
                    const f = new FormData(); f.append('id', id);
                    const r = await fetch(`../api/admin_manage.php?action=delete&type=${type}`, { method: 'POST', body: f });
                    const data = await r.json();
                    if(data.status === 'success') { 
                        toastr.success(`Data ${type} telah berhasil dihapus.`); 
                        loadMasterData();
                        if(type === 'size') bootstrap.Modal.getInstance(document.getElementById('modalSize'))?.hide();
                        if(type === 'quantity') bootstrap.Modal.getInstance(document.getElementById('modalQty'))?.hide();
                    } else {
        ...
        window.openMachineModal = (data=null) => {
            document.getElementById('formMachine').reset();
            document.getElementById('machine_id').value = data ? data.id : '';
            if(data) { document.getElementById('machine_name').value = data.name; document.getElementById('machine_status').value = data.status; }
            $('.default-select').selectpicker('refresh');
            cleanModal('modalMachine').show();
        };
        ...
        document.getElementById('formPin').onsubmit = (e) => handleAJAXSave(e, 'pin', 'modalPin');
        ...
        window.openPinModal = (data=null) => { document.getElementById('formPin').reset(); document.getElementById('pin_id').value = data ? data.id : ''; if(data) { document.getElementById('pin_code').value = data.pin_code; document.getElementById('pin_note').value = data.note; } cleanModal('modalPin').show(); };
        ...
        window.openTemplateModal = (data=null) => { 
            ...
            if(data) { 
                ...
                document.getElementById('list_sizes').innerHTML = '';
                document.getElementById('list_qtys').innerHTML = '';
            }
            $('.default-select').selectpicker('refresh');
            cleanModal('modalTemplate').show(); 
        };
        ...
        loadMasterData();
        document.querySelector('a[href="pengaturan.php"]')?.closest('li')?.classList.add('mm-active');
    </script>
</body>
</html>
