    ...
    try { 
      ...
      await db.transaction((txn) async {
        for (var r in remoteData) {
          ...
          if (local.isEmpty) {
            neededHealing = true;
            await txn.insert('queue', {'item': r['item']?.toString() ?? '', 'size': r['size']?.toString() ?? '', 'unit': r['unit']?.toString() ?? '', 'batch': b, 'machine': r['machine']?.toString() ?? '', 'shift': r['shift']?.toString() ?? '', 'quantity': r['quantity']?.toString() ?? '', 'operator': r['operator']?.toString() ?? '', 'qc': r['qc']?.toString() ?? '', 'production_date': r['production_date']?.toString() ?? '', 'production_time': r['production_time']?.toString() ?? '', 'copies': remoteCopies, 'is_printed': 1, 'is_synced': 1, 'device_model': r['device_model']});
          } else {
            int localCopies = local.first['copies'] as int;
            if (remoteCopies > localCopies) { neededHealing = true; await txn.update('queue', {'copies': remoteCopies, 'device_model': r['device_model']}, where: 'id = ?', whereArgs: [local.first['id']]); }
          }
        }
      });
